...
class TodayViewModel @Inject constructor (
    ...
    fun newTask(
        ...
        viewModelScope.launch(Dispatchers.IO) {
            FirebaseCrashlytics.getInstance().log("Creating an issue")
            val requestTask = ApiRequestTask(description, time = "${LocalDate.now()}T${time}")
            val token = getSavedTokenUseCase.invoke()
            FirebaseCrashlytics.getInstance().setCustomKey("user_token", token ?: "null")
            if(token != null){
                when(val result = postNewTaskUseCase.invoke(
                    ...
                    is NetworkResult.Error -> {
                        val message = "Failed to add task: ${result.message}"
                ...
                val message = "Authentication error: Token is null"
                FirebaseCrashlytics.getInstance().log(message)
                showToast(message)
            }
        }
    ...
    private fun loadUserTodayTasks() {
        viewModelScope.launch(Dispatchers.IO) {
            FirebaseCrashlytics.getInstance().log("Loading tasks")
            val token = getSavedTokenUseCase.invoke()
            FirebaseCrashlytics.getInstance().setCustomKey("user_token", token ?: "null")
            if(token != null){
                when(val result = getTasksByUserAndDateUseCase.invoke(
                    ...
                    is NetworkResult.Error -> {
                        val message = "Error loading tasks: ${result.message}"
                        FirebaseCrashlytics.getInstance().recordException(Exception())
                        FirebaseCrashlytics.getInstance().log(message)
                        showToast(message)
                    }
    ...
    fun loadTaskDetails(taskId: Long) {
        viewModelScope.launch(Dispatchers.IO) {
            FirebaseCrashlytics.getInstance().log("Uploading task details")
            val token = getSavedTokenUseCase.invoke()
            FirebaseCrashlytics.getInstance().setCustomKey("user_token", token ?: "null")
            if (token != null) {
                when (val result = getTaskDetailsUseCase.invoke(token, taskId)) {
                    ...
                    is NetworkResult.Error -> {
                        val message = "Error loading task details: ${result.message}"
                        FirebaseCrashlytics.getInstance().recordException(Exception())
                        FirebaseCrashlytics.getInstance().log(message)
                        showToast(message)
                    }
    ...
    fun deleteTask(taskId: Long) {
        viewModelScope.launch(Dispatchers.IO){
            FirebaseCrashlytics.getInstance().log("Deleting a task")
            val token = getSavedTokenUseCase.invoke()
            FirebaseCrashlytics.getInstance().setCustomKey("user_token", token ?: "null")
            if(token != null){
                when(val result = postDeleteTaskUseCase.invoke(token, taskId)){
                    ...
                    is NetworkResult.Error -> {
                        val message = "Error deleting a task: ${result.message}"
                        FirebaseCrashlytics.getInstance().recordException(Exception())
                        FirebaseCrashlytics.getInstance().log(message)
                        showToast(message)
                    }
